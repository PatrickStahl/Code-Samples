...
{
    // Bereite Download mit spezifischen Einstellungen für die FSU-Jena vor
    public boolean download(String username, String host, String email, String password, Integer port) throws MessagingException, IOException {
        ...
        // Index hier nur auf 30 zum Testen gestellt, bei Bedarf auf messages.length für alle Mails stellen
        for (int i = folder.listFiles().length; i < 30; i++)
        {
            try
            {
    ...
    // Erstelldatum der Mails wird auf ihr Empfangsdatum gesetzt um sie chronologisch auflisten zu können
    ...
    // Header wird hinzugefügt, um zwischen gelesenen und ungelesenen unterscheiden zu können
    ...
    // Nachrichtenkörper entsprechend des Encodings geladen
    public String loadBody(String directory, String subject) throws MessagingException, IOException
    {
        ...
        if (message.getContent() instanceof MimeMultipart)
        {
            ...
            for (int i = 0; i < mimeMultipart.getCount(); i++)
            {
                ...
                {
                    // Wenn Text MultiPart/Alternative ist, gib den ersten body-part aus
    ...
    // Hilfsfunktion um Absender zu extrahieren
        ...
        if (text.startsWith("=?"))
        {
            ...
            for (int i = 0; i < splits.length; i++)
            {
                ...
                try
                {
                    ...
                    {
                        encodedText = encodedText.replaceAll("=([0-9A-Fa-f]{2})", "%$1"); // Ersetze alle "=XX" mit "%XX"
    ...
    // Analog zu Absender; extrahiere Empfänger
    ...
    // Extrahiere Datum im deutschen Format
    ...
    // Selbe Funktion aber für Dateibenennung
    ...
    // Abfrage des Headers (gelesen/ungelesen)
