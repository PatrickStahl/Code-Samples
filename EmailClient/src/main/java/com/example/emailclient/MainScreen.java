...
public class MainScreen {
    ...
    // lädt Mails vom Server herunter und aktualisiert die Tabelle
    @FXML
    void refreshButtonClicked() throws MessagingException, IOException {
        ...
        // Schließe Fenster, falls download fehlschlägt
        if(!checkIfContinue)
        {
    ...
    // öffne Fenster zum Versenden von Mails
    @FXML
    ...
    // Initialisiere Tabelle beim Öffnen des Fensters
    ...
    private void initiateCols() {
        ...
        columnSubject.setCellFactory(tc ->
        {
            ...
            // Öffne Mail beim anklicken
            cell.setOnMouseClicked(e ->
            {
                // Öffne passende Mail anhand der Tabellenspalte
    ...
    // Lade alle Mails des lokalen Ordners
    private void loadData() throws MessagingException, IOException {
        ...
        if (listOfFiles != null) {
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                if (listOfFiles[i].isFile()) {
                    ...
                    Mail currentMail = new Mail(
                            saveLoad.getSubject(username, name),
                            saveLoad.getFrom(username, name),
                            saveLoad.getTo(username, name),
                            saveLoad.getDate(username, name),
                            saveLoad.getHeaderValue(username, name)
                    );
            ...
            table.setItems(list);
            table.refresh();
            // Nach erfolgreicher aktualisierung Statuslabel leeren
            labelStatus.setText("");
        }

    }

    // Funktion zum Laden der einzelnen Mail im Detailfenster
    public void loadMail(String subject, String from, String to, String body) throws IOException {
        ...
        openMail.loadContent(subject, from, to, body);
        Stage stage = new Stage();
    ...
    // Hilfsklasse zur Darstellung der Emails
    public static class Mail {
        ...
        public String getSeenHeader() { return seenHeader.get(); }
